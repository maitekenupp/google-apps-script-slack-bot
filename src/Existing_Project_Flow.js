function showExistingProjectRoleSelect_(userId, channelId, messageTs) {
  ...
  try {
    const projects = loadProjectsNeedingRoles_();
...
function showExistingProjectContractorSelect_(userId, channelId, messageTs) {
  ...
  try {
    const projects = loadProjectsReadyForContractors_();
...
function handleExistingProjectRolesSelect_(payload) {
  ...
  const project = getNotionProjectSummary_(selectedProjectId);
...
function handleExistingProjectContractorsSelect_(payload) {
  ...
  const project = getNotionProjectSummary_(selectedProjectId);
  const projectRoles = loadProjectRolesForAssignment_(selectedProjectId);
  ...
  const session = {
    ...
    roleDrafts: projectRoles.map(role => ({
      roleName: role.role,
...
function loadProjectsNeedingRoles_() {
  const projectRows = queryAllDataSourceRows_(PROJECTS_OVERVIEW_DATA_SOURCE_ID);
  const taskRows = queryAllDataSourceRows_(TASKS_DATA_SOURCE_ID);

  const projectsWithRoles = {};

  taskRows.forEach(row => {
    const projectIds = getRelationIds_(row.properties["Project"]);
  ...
  return projectRows
    ...
    .map(row => ({
      id: row.id,
      name: getText_(row.properties["Project Name"]) || "Untitled Project",
      status: getText_(row.properties["Project Status"])
    }))
    .filter(project =>
      project.name &&
      !["Done", "Canceled"].includes(project.status)
    )
...
function getNotionProjectSummary_(projectId) {
  ...
  return {
    id: row.id,
    name: getText_(p["Project Name"]) || "Untitled Project",
    description: getText_(p["Short Description"]) || "",
    startDate: getDateStart_(p["Project Start Date"]),
    endDate: getDateStart_(p["Project End Date"])
  };
}
